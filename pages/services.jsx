...
class ModelsPage extends React.Component {
    ...
    static async getInitialProps (ctx) {
        ...
        let models = await axios.get(`${apiHost}/public/model/list`);
        return { authToken, servicesList: _.keyBy(servicesList, 'alias'), modelList: _.orderBy(_.reject(models.data, { is_group: 1 }), 'markName') };
    }
                            ...
                            <div className="list blocks">
                                ...
                                <div className="isActive">
                                    {_.map(this.props.modelList, i=><div className={(_.includes((this.state.servicesList[this.state.currentService] || {}).models, i.alias))?'blue':''} key={i.alias} onClick={()=>this.setServiceCar(i.alias)}>
                                        <div>{i.markName}</div>
                                        <div className="bold">{i.modelName}</div>
                                    </div>)}
                                </div>
                            </div>
                        :null}
